import pandas as pd
import openai
import time

# Set your OpenAI API key
openai.api_key = ""

# Function to generate a product description
def generate_description(product_title):
    try:
        # Create a chat-based API request
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",  # Using gpt-3.5-turbo model
            messages=[
                {"role": "system", "content": "You are a helpful assistant."},
                {"role": "user", "content": f"Generate a short, creative and engaging description between 50-100 words for the following title: '{product_title}'."}
            ]
        )
        # Return the generated description from the response
        return response.choices[0].message['content'].strip()

    except Exception as e:
        print(f"Error generating description for '{product_title}': {e}")
        raise e


# Main function to process the Excel file
def process_excel(file_path):
    try:
        df = pd.read_excel(file_path)

        if 'TITLE' not in df.columns:
            raise ValueError("The column 'TITLE' does not exist in the Excel file.")

        if 'DESCRIPTION' not in df.columns:
            df['DESCRIPTION'] = ""

        generation_count = 10281

        for index, row in df.iterrows():
            title = row['TITLE']
            description = row['DESCRIPTION']

            # Skip rows with missing titles and check for an empty row at the end
            if pd.isna(title) or title.strip() == "":
                print(f"Skipping empty title at row {index + 2}.")  # +2 for Excel row numbering

                # Check if it's the last row with an empty title
                if index == len(df) - 1:
                    print("Last row is empty. Terminating process.")
                    break
                continue

            # If the description is empty, generate a new description
            if pd.isna(description) or description.strip() == "":
                try:
                    generation_count += 1
                    print(f"<<{ generation_count }>> Generating description for: {title}")
                    description = generate_description(title)
                    df.at[index, 'DESCRIPTION'] = description

                    print(f"Success: Description for '{title}' generated and stored.")

                    # Save the file after each successful description
                    df.to_excel(file_path, index=False)
                    print("Excel file updated successfully...!!!")
                    print("<=================================================================================================>")

                    # Wait 2 minutes before the next API call
                    print("Cooldown few seconds before the next API call...!!!")
                    time.sleep(3)

                except Exception as e:
                    print(f"Failure: Could not generate description for '{title}'. Stopping process.")
                    break

            else:
                print(f"Description already exists for '{title}', skipping description generation.")

        # After processing, print the total number of generated descriptions
        print(f"Total descriptions generated: {generation_count}")

    except Exception as e:
        print(f"Error processing Excel file: {e}")


# Main execution
if __name__ == "__main__":
    excel_file_path = "Output.xlsx"
    process_excel(excel_file_path)




# import pandas as pd
# import openai
# import time
#
# # Set your OpenAI API key
# openai.api_key = "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"
#
# # Function to generate a product description
# def generate_description(product_title):
#     try:
#         # Create a chat-based API request
#         response = openai.ChatCompletion.create(
#             model="gpt-3.5-turbo",  # Using gpt-3.5-turbo model
#             messages=[
#                 {"role": "system", "content": "You are a helpful assistant."},
#                 {"role": "user", "content": f"Generate a short, creative and engaging description between 50-100 words for the following title: '{product_title}'."}
#             ]
#         )
#         # Return the generated description from the response
#         return response.choices[0].message['content'].strip()
#
#     except Exception as e:
#         print(f"Error generating description for '{product_title}': {e}")
#         raise e
#
#
# # Main function to process the Excel file
# def process_excel(file_path):
#     try:
#         # Load the Excel file
#         df = pd.read_excel(file_path)
#
#         # Ensure the 'TITLE' column exists
#         if 'TITLE' not in df.columns:
#             raise ValueError("The column 'TITLE' does not exist in the Excel file.")
#
#         # Ensure 'DESCRIPTION' column exists, or create it
#         if 'DESCRIPTION' not in df.columns:
#             df['DESCRIPTION'] = ""
#
#         # Initialize a counter for description generations
#         generation_count = 5767
#
#         for index, row in df.iterrows():
#             title = row['TITLE']
#             description = row['DESCRIPTION']
#
#             # Skip rows with missing titles and check for an empty row at the end
#             if pd.isna(title) or title.strip() == "":
#                 print(f"Skipping empty title at row {index + 2}.")  # +2 for Excel row numbering
#
#                 # Check if it's the last row with an empty title
#                 if index == len(df) - 1:
#                     print("Last row is empty. Terminating process.")
#                     break
#                 continue
#
#             # If the description is empty, generate a new description
#             if pd.isna(description) or description.strip() == "":
#                 try:
#                     generation_count += 1
#                     print(f"<<{ generation_count }>> Generating description for: {title}")
#                     description = generate_description(title)
#                     df.at[index, 'DESCRIPTION'] = description
#
#                     print(f"Success: Description for '{title}' generated and stored.")
#
#                     # Save the file after each successful description
#                     df.to_excel(file_path, index=False)
#                     print("Excel file updated successfully...!!!")
#                     print("<=================================================================================================>")
#
#                     # Wait 2 minutes before the next API call
#                     print("Cooldown few seconds before the next API call...!!!")
#                     time.sleep(3)
#
#                 except Exception as e:
#                     print(f"Failure: Could not generate description for '{title}'. Stopping process.")
#                     break
#
#             else:
#                 print(f"Description already exists for '{title}', skipping description generation.")
#
#         # After processing, print the total number of generated descriptions
#         print(f"Total descriptions generated: {generation_count}")
#
#     except Exception as e:
#         print(f"Error processing Excel file: {e}")
#
#
# # Main execution
# if __name__ == "__main__":
#     excel_file_path = "v2.xlsx"  # Replace with your Excel file path
#     process_excel(excel_file_path)




# import openai
#
# # Set your OpenAI API key
# openai.api_key = "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"
#
# # Function to test API functionality
# def test_openai_api():
#     try:
#         # Make a simple request to the API
#         response = openai.ChatCompletion.create(
#             model="gpt-3.5-turbo",
#             messages=[
#                 {"role": "system", "content": "You are a helpful assistant."},
#                 {"role": "user", "content": "Write a short description for a product titled 'titan watch'."}
#             ]
#         )
#
#         # Extract and print the response content
#         description = response.choices[0].message.content.strip()
#         print("Generated Description:")
#         print(description)
#
#     except Exception as e:
#         print(f"API Error: {e}")
#
# # Run the function
# if __name__ == "__main__":
#     test_openai_api()
